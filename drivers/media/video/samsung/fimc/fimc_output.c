				   ...
				   struct fimc_ctx *ctx)
{
	...
	for (i = 0; i < FIMC_OUTBUFS; i++) {
#ifdef SYSMMU_FIMC
		ctx->dst[i].base[FIMC_ADDR_Y] = (int)phys_to_virt(*curr);
#else
				       ...
				       struct v4l2_pix_format *pixfmt)
{
	...
	if (pdata->hw_ver >= 0x52)
		fimc_hwset_output_rgb16b_fmt(ctrl, pixfmt->pixelformat);
...
int fimc_reqbufs_output(void *fh, struct v4l2_requestbuffers *b)
{
	...
	if (b->count == 0) {
		...
		if (b->memory == V4L2_MEMORY_MMAP) {
			...
			fimc_err("fimc_outdev_set_src_buf done");
			if (ret)
				return ret;
		}
		ctx->is_requested = 1;
			     ...
			     struct fimc_buf *fimc_buf)
{
	...
	for (i = 0; i < ctx->buf_num; i++) {
		buf = &fimc_buf[i];
#ifdef SYSMMU_FIMC
		vcm_res = (struct vcm_res *)ump_dd_meminfo_get( 	\
				(unsigned int)buf->base[FIMC_ADDR_Y], (void*)ctrl->vcm_id);
...
int fimc_cropcap_output(void *fh, struct v4l2_cropcap *a)
{
	...
	default:
		fimc_warn("Supported format : V4L2_PIX_FMT_YUYV, "
			...
			"V4L2_PIX_FMT_RGB555, V4L2_PIX_FMT_RGB444\n");
		return -EINVAL;
	}
				     ...
				     u32 idx, dma_addr_t *addr)
{
	...
	if (ctx->pix.pixelformat == V4L2_PIX_FMT_NV12T) {
		vcm_res = (struct vcm_res *)
			ump_dd_meminfo_get(addr[FIMC_ADDR_Y], (void*)ctrl->vcm_id);
		if(vcm_res)
			ctx->src[idx].base[FIMC_ADDR_Y] = vcm_res->start;
		else
			return -EINVAL;
		vcm_res = (struct vcm_res *)
			ump_dd_meminfo_get(addr[FIMC_ADDR_CB], (void*)ctrl->vcm_id);
		if(vcm_res)
